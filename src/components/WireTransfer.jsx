...
const [ledgerAccountBalance] = useState(parseFloat(localStorage.getItem('ledgerAccountBalance')) || 0);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://159.65.31.191:4040/api/Wallet/transfer', requestBody);
  ...
  <p className="text-right mt-2">
    Balance: ${selectedBalance.toFixed(2)}
              ...
              <input type="text" placeholder="Account Number" value={receiverWalletAddress} onChange={(e) => setReceiverWalletAddress(e.target.value)} className="border border-gray-300 p-2 rounded" />
              <input type="text" placeholder="Bank Name" value={receiverWalletAddress} onChange={(e) => setReceiverWalletAddress(e.target.value)} className="border border-gray-300 p-2 rounded" />
              ...
              <input type="number" placeholder="Swift Code" value={amount} onChange={(e) => setAmount(e.target.value)} className="border border-gray-300 p-2 rounded" />
